...
- Objective: Make the source-grounded `[ ]` backlog in `PLANNING.md` explicitly cover Hermes-style delegated-toolset restriction semantics.
...
- If `PLANNING.md` explicitly includes Hermes-style delegated-toolset restriction items, the implementation backlog will better match the reviewed Hermes operator workflow instead of leaving child-tool narrowing implicit.

## Steps

1. Re-read the current memory and the existing planning backlog.
2. Make one bounded change by adding Hermes-style delegated-toolset restriction items to `PLANNING.md`.
