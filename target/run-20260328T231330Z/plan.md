...
- Objective: Align root CLI help with the current safe `status.hermes_parity` surface by exposing `review_basis` in the built-in help text.
...
- If root CLI help explicitly includes `review_basis`, operators will be able to discover the full safe parity surface from built-in help without opening README or raw `status` JSON.

## Steps

1. Re-read the current memory, the current parity surface, and the root-help text.
2. Make one bounded change by documenting `review_basis` in root help.
