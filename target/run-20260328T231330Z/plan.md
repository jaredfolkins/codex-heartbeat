...
- Objective: Align the README parity explanation with the current safe `status.hermes_parity` surface by documenting `claim_rule` and the Hermes-style delegated cross-review gap.
...
- If the README parity section explicitly includes both `claim_rule` and the Hermes-style delegated cross-review gap, operators will be able to interpret the current non-parity answer from docs without inspecting the raw `status` JSON.

## Steps

1. Re-read the current memory, the current parity surface, and the README status section.
2. Make one bounded change by documenting `claim_rule` and the Hermes-style delegated cross-review gap in README.
